...
const initialMessages: Message[] = [
  {
    id: "msg-1",
    sender: "ai",
    text: "Hello! I am **Ask LOOP**, your grounded AI Customer Feedback Assistant. I have indexed **548 feedback items** in your workspace. Ask me anything about customer complaints, feature requests, or sentiment trends, and I will provide answers grounded directly in real customer quotes.",
    timestamp: "10:00 AM",
  },
  {
    id: "msg-2",
    sender: "user",
    text: "What are users saying about our onboarding experience?",
    timestamp: "10:01 AM",
  },
  {
    id: "msg-3",
    sender: "ai",
    text: "Based on 42 indexed feedback items regarding onboarding, customers are experiencing friction primarily around **team invitation flows** and **outdated setup documentation**. 78% of onboarding feedback carries a negative sentiment score.",
    citations: [
      {
        id: "fb-101",
        customer: "Sarah Jenkins (Stripe)",
        quote: "Onboarding took forever — I couldn't figure out how to invite my team. The docs were outdated.",
        channel: "Support Ticket",
        sentiment: "NEG",
      },
      {
        id: "fb-109",
        customer: "Dev Lead (Vercel)",
        quote: "Inviting 5 members during initial setup timed out twice before succeeding.",
        channel: "NPS Survey",
        sentiment: "NEG",
      },
    ],
...
const suggestedPrompts = [
  ...
  "Summarize main complaints about billing",
  "What is driving positive sentiment on v2?",
];
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const aiMsg: Message = {
        id: `msg-${Date.now() + 1}`,
        sender: "ai",
        text: `Based on semantic vector retrieval across your workspace feedback, here is what customers are stating regarding "${promptText}":\n\n- Customers highlight high interest in performance and streamlined UI workflows.\n- 85% of feedback items retrieved confirm positive reception on recent performance patches.`,
        citations: [
          {
            id: "fb-102",
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800/80 pb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Natural language Q&A grounded strictly in vector-retrieved customer quotes. No AI hallucinations.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 text-xs font-mono px-3 py-1.5 rounded-full bg-emerald-500/10 text-emerald-300 border border-emerald-500/30">
          ...
          <span>pgvector Indexing Active</span>
        </div>
      </div>
      ...
      <Card className="p-6 space-y-6 min-h-[500px] flex flex-col justify-between">
                ...
                {msg.citations && msg.citations.length > 0 && (
                  <div className="pt-3 border-t border-slate-800 space-y-2">
                    <p className="text-[11px] font-bold text-emerald-400 uppercase tracking-wider flex items-center gap-1 font-mono">
                      <Quote className="w-3.5 h-3.5" /> Cited Evidence ({msg.citations.length} Verified Quotes)
                    </p>
          ...
          {loading && (
            <div className="flex gap-4 items-center text-slate-400 text-xs">
              ...
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 flex items-center gap-3">
                ...
                <span>Performing vector similarity search & generating grounded response...</span>
              </div>
            </div>
          )}
        </div>
        ...
        <form onSubmit={handleSend} className="flex gap-3 pt-4 border-t border-slate-800">
          <input
            ...
            placeholder="Ask a plain-English question about customer feedback..."
